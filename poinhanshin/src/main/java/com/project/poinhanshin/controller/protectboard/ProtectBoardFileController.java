...
public class ProtectBoardFileController {
    ...
    @Autowired
    public ProtectBoardFileController(ProtectBoardFileService protectBoardFileService) {
    ...
    @ResponseBody
    public ResponseEntity<List<String>> read(Integer protectboardno) {
        ...
        return new ResponseEntity<List<String>>(storedFileNameList, HttpStatus.OK);
    }

    // 첨부 파일 삭제
    @DeleteMapping("/protectboard/file/{stored_file_name}") // /protectboard/file/{stored_file_name}?=Hello.jpg;
    @ResponseBody
    public ResponseEntity<String> remove(@PathVariable String stored_file_name) {

        try{
            if( protectBoardFileService.deleteFile(stored_file_name) != 1 )
